...
public class Benchmark_Timer<T> implements Benchmark<T> {
    ...
    public static void main(String[] args)
    {
        ...
        Benchmark_Timer<Integer[]> benchmarkTimer=new Benchmark_Timer<>
                ("Benchmarking",null,(i)->insertionSort.sort(i,0,i.length),null);

        System.out.println("--------Randomly Ordered Array-------");
        for(int i=200;i<=7000;i*=2)
        {
            int len=i;
            //creating supplier of type Integer[] and creating an array using Random
            ...
            System.out.println("Randomly Ordered: Value of N: " + i + " Time Taken: " + time);
        }
        ...
        for(int i=200;i<=7000;i*=2) {
            ...
            double time = benchmarkTimer.runFromSupplier(supplier, 20);
            System.out.println("Reverse Ordered: Value of N: " + i + " Time Taken: " + time);
        }
        ...
        for(int i=200;i<=7000;i*=2) {
            ...
            double time = benchmarkTimer.runFromSupplier(supplier, 20);
            System.out.println("Ordered Array: Value of N: " + i + " Time Taken: " + time);
        }
        ...
        for(int i=200;i<=7000;i*=2) {
            ...
            Supplier<Integer[]> supplier = new Supplier<Integer[]>() {
                ...
                public Integer[] get() {
                    ...
                    Arrays.sort(arr);
                    for(int k=0; k<(0.5*len);k++)
                    {
                        arr[k]=random.nextInt((int) (0.5*len)); //randomizing 1st half of the array
                    }
                    return arr;
                }
            };
            double time = benchmarkTimer.runFromSupplier(supplier, 20);
            System.out.println("Partially Ordered: Value of N: " + i + " Time Taken: " + time);
        }
    }
}
